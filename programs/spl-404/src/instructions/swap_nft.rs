...
pub fn swap_nft(ctx: Context<SwapNft>, args: SwapNftArgs) -> Result<()> {
   ...
   transfer_checked(CpiContext::new(cpi_program.clone(), cpi_fee_accounts), 200 * 10u64.pow(ctx.accounts.mystery_box.decimals as u32), ctx.accounts.token_mint.decimals)?;
   ...
   transfer_checked(CpiContext::new(cpi_program.clone(), cpi_nft_accounts), 1, ctx.accounts.nft_mint.decimals)?;
  ...
  let signer: &[&[&[u8]]] = &[&[
   ...
   transfer_checked(CpiContext::new_with_signer(cpi_program.clone(), cpi_nft_accounts, signer), ctx.accounts.mystery_box.token_per_nft * 10u64.pow(ctx.accounts.mystery_box.decimals as u32), ctx.accounts.token_mint.decimals)?;
